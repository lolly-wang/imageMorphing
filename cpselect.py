...
def cpselect(img1, img2):
    ...
    point_left = np.concatenate([(np.array(point.left_x) * resize_img1.shape[1] * 1.0 / 300)[..., np.newaxis], \
                                 (np.array(point.left_y) * resize_img1.shape[0] * 1.0 / 300)[..., np.newaxis]], axis=1)
    point_right = np.concatenate([(np.array(point.right_x) * resize_img2.shape[1] * 1.0 / 300)[..., np.newaxis], \
                                  (np.array(point.right_y) * resize_img2.shape[0] * 1.0 / 300)[..., np.newaxis]], axis=1)
